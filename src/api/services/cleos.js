...
exports.createKey = async () => {
  try {
    ...
    const { stdout, stderr } = await exec(`~/eos/build/programs/cleos/cleos create key`);
    if (stderr) {
      1;
    }
...
exports.createAccount = async (creatorAccount, accountName, ownerPublicKey, activePublicKey) => {
  try {
    ...
    //await exec(`sudo ~/eos/build/programs/cleos/cleos wallet open`);
    const { stdout, stderr } = await exec(`sudo ~/eos/build/programs/cleos/cleos create account eosio ${accountName} ${ownerPublicKey} ${activePublicKey}`);
    if (stderr) {
      throw stderr;
    }
    return true;
...
exports.createWallet = async (walletName) => {
  ...
  const url = `http://localhost:8888/v1/wallet/create`
